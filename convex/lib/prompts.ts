...
// Enhanced base style prompt with animation smoothness emphasis
...
export const DIRECTION_PROMPTS_8: Record<Direction, string> = {
...
// Legacy 4-direction prompts for backwards compatibility
...
/**
 * Generate 4-direction sheet prompt for split generation (Option B)
...
export function get4DirectionalSheetPrompt(
  ...
  const directions = directionSet === 'cardinal' ? CARDINAL_DIRECTIONS : DIAGONAL_DIRECTIONS;
  const directionLabels = directions.map(d => DIRECTION_PROMPTS_8[d]);
  
  const sheetWidth = config.columns * FRAME_SIZE.width;
  const sheetHeight = 4 * FRAME_SIZE.height; // 4 rows for 4 directions
...
ANIMATION SMOOTHNESS - CRITICAL:
- Frame 1 → Frame 2 → Frame 3... must flow like a video slowed down
...
- Diagonals show proper 45° angle, not just rotated versions
...
/**
 * Legacy 4-direction sheet prompt (backwards compatible)
 */
...
/**
 * Enhanced single-direction animation prompt for directional types
 */
export function getDirectionalAnimationPrompt(
  ...
  const config = ANIMATION_CONFIGS[animationType];
  const directionView = DIRECTION_PROMPTS_8[direction];
  const timing = animationType === 'idle' ? ANIMATION_TIMING.IDLE : ANIMATION_TIMING.WALK;
...
/**
 * Enhanced combat animation prompts with more frames and smoother descriptions
 */
...
Frame 5: Mid-fall - body at 45° angle, arms limp
...
/**
 * Get animation prompt for any type
 */
...
/**
 * Combined attack sheet prompt (3 rows = 3 attack types)
 */
 ...
 * Returns array of [cardinalPrompt, diagonalPrompt]
 */
